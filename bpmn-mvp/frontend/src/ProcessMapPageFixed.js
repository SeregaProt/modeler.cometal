...
export default function ProcessMapPage({ projectId, onBack, highlightedProcessId, onOpenProcess }) {
  ...
  const createConnection = async (sourceNodeId, targetNodeId) => {
    ...
    try {
      ...
      const existingEdge = edges.find(e => 
        (e.source === String(sourceNodeId) && e.target === String(targetNodeId)) ||
        (e.source === String(targetNodeId) && e.target === String(sourceNodeId))
      );
      
      if (existingEdge) {
        console.warn('⚠️ Связь уже существует:', existingEdge);
      ...
      console.log('📍 Добавляем новую связь:', { newEdge, fullRelation });
  ...
  useEffect(() => {
    ...
    console.log('🎯 Состояние создания связей:', { isConnecting, connectionSource, isDragging });
    ...
    const newNodes = processes.map((proc, index) => {
      ...
      console.log(`📊 Узел ${proc.id} данные:`, {
        isConnecting: nodeData.isConnecting,
        showHoverPoints: nodeData.showHoverPoints,
        hasOnStartConnection: !!nodeData.onStartConnection
      });
    ...
    setNodes(newNodes);
  }, [processes, foundIds, highlightedProcessId, selectedNodeId, hoveredNodeId, isDragging, isConnecting, connectionSource?.nodeId, handleStartConnection, handleStartDrag, handleNodeMouseEnter, handleNodeMouseLeave]);

  // Обновление связей при изменен��и данных
  useEffect(() => {
    ...
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      >
        ...
        <Typography variant="body2" sx={{ mt: 2, color: 'text.secondary' }}>
          ...
          <br />• Синие точки появляются при выделении процесса
      ...
      <Dialog open={deleteProcessDialogOpen} onClose={() => !isDeletingProcess && setDeleteProcessDialogOpen(false)}>
        ...
        <DialogContent>
          {processToDelete && (
            <Typography>
              Вы действи��ельно хотите удалить процесс "{processToDelete.name}"?
            </Typography>
          )}
        </DialogContent>
        <DialogActions>
